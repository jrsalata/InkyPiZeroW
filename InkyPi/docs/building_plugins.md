...
3. This function posts the rendered HTML to the renderer backend (`POST /upload/<width>/<height>`), where Chromium in headless mode captures a screenshot and returns it as PNG.

### Renderer Backend URL

- The renderer backend base URL is configured with the `INKYPI_RENDERER_URL` environment variable.
- If unset, it defaults to `http://127.0.0.1:8000`.
- For reliability, templates should be self-contained (inline CSS/fonts/JS/local images) so the renderer backend does not depend on local frontend files.
